import convertFirestoreTimestampsToDates from '@/packages/helpers/convertFirestoreTimestampsToDates';
import { Timestamp } from '@/../tests/testHelpers';

describe('helpers/convertFirestoreTimestampsToDates', () => {
  const date = new Date('2015-12-23 22:33:44');
  const timestamp = Timestamp.fromDate(date);

  it.skip('converts Firestore Timestamp objects into equivalent Date objects', () => {
    const data = {
      name: 'John Smith',
  ...
  it.skip('converts deeply nested Timestamps', () => {
    const data = {
      some: {
        deeply: {
          nested: {
